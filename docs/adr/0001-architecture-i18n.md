...
**Correction (2026-09-09) : l'inventaire ci-dessus avait un angle mort — il ne pouvait pas voir ce qu'il n'a jamais lu.** L'inventaire « union de toutes les configurations déjà utilisées » a été construit en lisant uniquement les schémas du dépôt (`studio/schemaTypes/`) — jamais le contenu réel stocké dans `production`. Il ne pouvait donc pas savoir que `careerPage` avait déjà, depuis mai 2026, son propre type `localizedBlock` déployé directement en production par le porteur de projet, indépendamment de ce chantier — un type au bloc entièrement libre (`type: 'block'` sans restriction), sous lequel du contenu réel avait été rédigé avec des titres `h4`/`h5`. Quand le schéma de cette branche a fini par être déployé en production (phase 5B), il a remplacé cette définition plus permissive par celle-ci — même nom de type, configuration plus étroite — et les titres `h4`/`h5` déjà écrits sont devenus des styles que l'éditeur Studio ne reconnaît plus. Le contenu lui-même n'a jamais été perdu ni modifié (vérifié : le document `careerPage` en production est resté identique, champ par champ, avant et après ce déploiement) ; seul l'éditeur du Studio ne pouvait plus l'afficher correctement — le site public, qui ne lit jamais le schéma du Studio, l'a continué d'afficher sans interruption. Un audit direct des données de production (GROQ, lecture seule) a trouvé exactement deux styles utilisés mais non déclarés (`h4` dans `teamPage.joinUsText`, `h5` dans `teamPage.messageText` et dans les sections de `careerPage`) ; aucun écart similaire pour les listes, les marques ou les annotations, déjà correctement couvertes par l'inventaire d'origine.

**Leçon pour toute future modification de schéma partagé** : un inventaire « union des configurations déjà utilisées » construit uniquement à partir du dépôt de code suppose implicitement que le dépôt est la seule source de vérité sur ce qui existe — une hypothèse qui ne tient pas quand un schéma a pu être déployé et modifié directement en production, hors du dépôt, comme ce fut le cas ici. Une vérification empirique contre les données réelles (comme celle qui a permis de trouver cet écart) reste nécessaire avant de considérer un tel inventaire complet.
