...
const MapFrameContainer = styled(motion.div)`
  padding: 10px;
...
export default function Transport({ data }) {
    ...
    const contactExtension = data?.contactExtension;
    ...
    const handleMouseMove = useCallback((event) => {
        ...
        // Find the path under the mouse
        const element = document.elementFromPoint(event.clientX, event.clientY);
        const path = element.closest('path');

        if (path && path.classList.contains("hoverMap")) {
            if (hoveredPath !== path) {
                // Reset previous hovered path
                if (hoveredPath) {
                    hoveredPath.style.fill = "";
                    hoveredPath.style.stroke = "";
                    hoveredPath.style.strokeWidth = "";
                    hoveredPath.style.zIndex = "";
                }

                // Set new hovered path
                setHoveredPath(path);
                const text = path.getAttribute("data-hover-text");
                const offsetX = parseFloat(path.getAttribute("data-hover-offset-x") || "0");
                const offsetY = parseFloat(path.getAttribute("data-hover-offset-y") || "-30");
                ...
                path.style.fill = "var(--secondary-darkcolor)";
                path.style.stroke = "white";
                path.style.strokeWidth = "3px";
                path.style.zIndex = "20";

                // Move the hovered path to the end of its parent <g> element
                const parentGroup = path.parentNode;
                parentGroup.appendChild(path);
            }
        } else {
            // Mouse is not over any path
            if (hoveredPath) {
                hoveredPath.style.fill = "";
                hoveredPath.style.stroke = "";
                hoveredPath.style.strokeWidth = "";
