...
describeOrSkip("GROQ locale-fallback pattern (live, staging dataset)", () => {
  ...
  beforeAll(async () => {
    ...
    await client.createOrReplace({
      ...
      strPresent: { _type: "localizedString", fr: "Texte francais", hyw: "Hayeren texte" },
      strEmpty: { _type: "localizedString", fr: "Texte francais", hyw: "" },
      strAbsent: { _type: "localizedString", fr: "Texte francais" },

      textPresent: { _type: "localizedText", fr: "Long texte francais", hyw: "Yerkar hayeren" },
      textEmpty: { _type: "localizedText", fr: "Long texte francais", hyw: "" },
      textAbsent: { _type: "localizedText", fr: "Long texte francais" },

      blockPresent: {
        _type: "localizedBlock",
        fr: block("Bloc francais"),
        hyw: block("Bloc hayeren"),
      },
      blockEmpty: { _type: "localizedBlock", fr: block("Bloc francais"), hyw: [] },
  ...
  describe.each([
    ...
    test("target locale present with real content returns target content", async () => {
      const v = await fetchField(presentField, fragmentBuilder, "hyw");
      expect(v).toBe(targetContent);
    });

    test("target locale present but empty falls back to French", async () => {
      const v = await fetchField(emptyField, fragmentBuilder, "hyw");
      expect(v).toBe(frContent);
      expect(v).not.toBe("");
    });

    test("target locale key absent entirely falls back to French", async () => {
      const v = await fetchField(absentField, fragmentBuilder, "hyw");
      expect(v).toBe(frContent);
    });
  });
